...
const AnswerByType = ({ responseData, requestData, interpretData, interpretModel, attackData, attackModel}) => {
  if(requestData && responseData) {
    ...
    if (requestData.model === 'NMN (trained on DROP)') {
      ...
      let questionStuff = []
      const questionPredicates = programExecution.reverse().reduce((acc, e) => {
        Object.keys(e).forEach(key => {
          if (e[key].question) {
            acc.push(e[key].question)
          }
        })
        return acc;
      }, [])
      for (let i = 0; i < questionTokens.length; i++) {
        questionStuff.push(questionPredicates.reduce((span, predicate, predicateIndex) => {
          return (
            <span style={{backgroundColor: `rgba(${predicateIndex === 0 ? 128: 0}, ${predicateIndex === 1 ? 189 : 0}, 255, ${predicate[i]})`, borderRadius: '4px', padding: `${(predicateIndex + 1) * 2}px`}}>
              {span}
            </span>
          )
        }, questionTokens[i]))
      ...
      let passageStuff = []
      const answerPredicates = programExecution.reverse().reduce((acc, e) => {
        console.log('answer', Object.keys(e))
        Object.keys(e).forEach(key => {
          if (e[key].passage) {

            acc.push(e[key].passage)
          }
        })
        return acc;
      }, [])
      for (let i = 0; i < passageTokens.length; i++) {
        passageStuff.push(answerPredicates.reduce((span, predicate, predicateIndex) => {
          return (
            <span style={{backgroundColor: `rgba(${predicateIndex === 0 ? 128: 0}, ${predicateIndex === 1 ? 189 : 0}, 255, ${predicate[i]})`, borderRadius: '4px', padding: `${(predicateIndex + 1) * 2}px`}}>
              {span}
            </span>
